...
class CrowdSafetyDashboard:
    ...
    def create_stat_panel(self, width=400, height=140):
        """Create statistics panel"""
        panel = np.zeros((height, width, 3), dtype=np.uint8)
        panel[:] = (26, 26, 26)  # Dark background
        
        # Title
        cv2.putText(panel, 'SYSTEM STATISTICS', (20, 30),
        ...
        for label, value in stats:
            ...
            cv2.putText(panel, value, (180, y_pos),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            y_pos += 30
            
        return panel
    
    def create_alert_panel(self, width=400, height=140):
        """Create alert/notification panel"""
        panel = np.zeros((height, width, 3), dtype=np.uint8)
        panel[:] = (26, 26, 26)
        
        # Title
        cv2.putText(panel, 'ACTIVE ALERTS', (20, 30),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        ...
        if not self.current_alerts:
            ...
            for alert in self.current_alerts[-3:]:  # Show last 3 alerts
                ...
                # Alert text
                cv2.putText(panel, alert['message'][:35], (40, y_pos),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
                y_pos += 30
                
        return panel
    
    def create_heatmap_legend(self, width=200, height=140):
        """Create a heatmap color legend"""
        panel = np.zeros((height, width, 3), dtype=np.uint8)
        panel[:] = (26, 26, 26)
        
        cv2.putText(panel, 'DENSITY MAP', (20, 25),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        
        # Color gradient
        gradient = np.zeros((70, 160, 3), dtype=np.uint8)
        for i in range(160):
            value = int((i / 160) * 255)
        ...
        panel[40:110, 20:180] = gradient
        
        # Labels
        cv2.putText(panel, 'Low', (20, 130), cv2.FONT_HERSHEY_SIMPLEX, 
                   0.4, (255, 255, 255), 1)
        cv2.putText(panel, 'High', (140, 130), cv2.FONT_HERSHEY_SIMPLEX, 
                   0.4, (255, 255, 255), 1)
    ...
    def render_dashboard(self, main_frame):
        ...
        # Bottom panels
        bottom_y = 940
        dashboard[bottom_y:bottom_y+140, 20:420] = stat_panel
        dashboard[bottom_y:bottom_y+140, 440:840] = alert_panel
        dashboard[bottom_y:bottom_y+140, 860:1060] = heatmap_legend
